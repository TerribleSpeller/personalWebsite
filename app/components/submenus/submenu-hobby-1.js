  ...
  return (
    <>
      <div className="tab-submenu">
        ...
        <button className={`tablinks-submenu rounded-4 ${activeTab === 'EJS' ? 'active' : ''}`} onClick={() => openSubTab('EJS')}><span className="highLight">EJS</span></button>
      ...
      <div id="py" className={`tabcontent-submenu ${activeTab === 'py' ? 'show' : ''}`}>
        <h3>Python</h3>
      ...
      <div id="EJS" className={`tabcontent-submenu ${activeTab === 'EJS' ? 'show' : ''}`}>
        <h3>EJS</h3>
      ...
      <div id="C++" className={`tabcontent-submenu ${activeTab === 'C++' ? 'show' : ''}`}>
        <h3>C++</h3>
      ...
      <div id="Kotlin" className={`tabcontent-submenu ${activeTab === 'Kotlin' ? 'show' : ''}`}>
        <h3>Kotlin</h3>
        <p>With IoT beocming significantly more important, mobile applications are a growing avenue that needs to be taken note of and exploited. So far, I have  {(currentMonth  > 11) ? `${currentYear - 2024} years` : `${currentMonth} months` } of experience with them, mainly with the utilization of Kotlin and Android Studio. Kotlin was the first language I learned that had the capbilities to be deployed on Android, and with it I learned how to deploy apps that can interact with the rest of the IoT network. Going so far to have one semester's final project to involve using one such application! </p>
      </div>

      <div id="PHP" className={`tabcontent-submenu ${activeTab === 'PHP' ? 'show' : ''}`}>
        <h3>PHP</h3>
